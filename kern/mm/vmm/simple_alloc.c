...
void * simple_alloc(size_t size, gfp_t flags)
{
	unsigned long flags;
...
/* Sanity check... */
	if (intr_count && flags != GFP_ATOMIC) {
		kprintf("kmalloc called nonatomically from interrupt %08lx\n",
...
#if 0
    kprintf ("Got page %08x to use for %d byte mallocs....",(long)page,sz);
#endif
    sizes[order].npages++;
...
if ((order < 0) || 
    ...
    (p->bh_flags != MF_USED))
    {
    kprintf ("kfree of non-kmalloced memory: %p, next= %p, order=%d\n",
...
if (size &&
    size != p->bh_length)
    {
    kprintf ("Trying to free pointer at %p with wrong size: %d instead of %lu.\n",
        p,size,p->bh_length);
    return;
    }
size = p->bh_length;
